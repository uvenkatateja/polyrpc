...
pub fn generate_definitions(types: &ExtractedTypes) -> String {
    ...
    // Generate enums first (they may be referenced by models)
...
fn py_type_to_ts(py_type: &PyType) -> String {
    match py_type {
        ...
        PyType::Generic(name) => name.clone(), // T, K, V etc.
...
mod tests {
    ...
    fn test_py_type_to_ts() {
        ...
        assert_eq!(
            py_type_to_ts(&PyType::GenericType("Response".to_string(), vec![PyType::Reference("User".to_string())])),
            "Response<User>"
        );
    }
